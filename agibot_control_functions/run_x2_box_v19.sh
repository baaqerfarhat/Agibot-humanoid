#!/usr/bin/env bash
# On-robot runner for box-walk v19 (iter 85500).
#
...
#   ./run_x2_box_v19.sh                 # dry-run
#   ./run_x2_box_v19.sh --engage        # publish (after the safety ladder)
...
cd ~/agibot_control_functions || {
  echo "missing ~/agibot_control_functions — run push_x2_box_v19_to_robot.sh from the laptop first"
  exit 1
}

POLICY="${POLICY:-policies/x2_box_policy_walk_retimed_v19_iter85500.npz}"
if [[ ! -f "$POLICY" ]]; then
  echo "missing policy: $POLICY"
  exit 1
fi

echo "===== v19 box-walk  policy=$POLICY  args: $* ====="
