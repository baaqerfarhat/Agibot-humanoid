#!/usr/bin/env python3
"""WORKING V1 (frozen 2026-09-15): the box-hug policy that ran ~90% on the robot on 2026-09-14/15.

Policy: policies/x2_boxhug_workingv1.npz (= x2_boxhug_policy_v40b2_iter49750.npz, run
physical_v40b2, iteration 49750, task Mjlab-BoxHug-Ref-X2-Robust). Runtime:
boxhug_runtime_workingv1.py (frozen copy). Do not edit this file or those two; keep
developing in deploy_x2_box_hug.py / boxhug_runtime.py. The mjlab-side freeze is
mjlab/logs/workingv1_box_hug/ (checkpoint, npz, params, eval video, notes).

    python3 deploy_x2_box_hug_workingv1.py            # dry run
    python3 deploy_x2_box_hug_workingv1.py --engage   # run on the robot

Deploy the mjlab X2 box-hug policy on the real AgiBot humanoid.
...
import boxhug_runtime_workingv1 as rt
...
def _default_policy_path() -> str:
    ...
    return os.path.join(here, "policies", "x2_boxhug_workingv1.npz")
...
def main():
    ...
    run_name = f"box_hug_workingv1_{os.path.splitext(os.path.basename(args.policy))[0]}"
    ...
    logger = RunLogger(
        ...
        meta={"script": "deploy_x2_box_hug_workingv1.py", "policy": args.policy,
