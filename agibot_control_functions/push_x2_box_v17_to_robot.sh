...
#
#   ping -c 1 10.0.1.41
#   ./agibot_control_functions/push_x2_box_v17_to_robot.sh
#
# Then SSH in and dry-run:
#   ssh -i ~/.ssh/agibot_ed25519 run@10.0.1.41
#   cd ~/agibot_control_functions && ./run_x2_box_v17.sh
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
HOST="${ROBOT_HOST:-run@10.0.1.41}"
KEY="${AGIBOT_SSH_KEY:-$HOME/.ssh/agibot_ed25519}"
POLICY_NAME="x2_box_policy_walk_feasible_v17_iter49000.npz"
...
"${SSH[@]}" "chmod +x agibot_control_functions/run_x2_box_v17.sh agibot_control_functions/_dryrun_box.sh"

echo "===== remote ls ====="
"${SSH[@]}" "ls -lh agibot_control_functions/policies/$POLICY_NAME agibot_control_functions/deploy_x2_box_pickup.py agibot_control_functions/run_x2_box_v17.sh"

echo
echo "On the robot:"
echo "  ssh ${KEY:+-i $KEY }$HOST"
echo "  aima em stop-app mc          # on 10.0.1.40, before any --engage"
echo "  cd ~/agibot_control_functions && ./run_x2_box_v17.sh"
echo "  # first --engage: SUSPENDED, NO BOX"
echo "  ./run_x2_box_v17.sh --engage"
