...
class EvalRecordingCallback(RLEvalCallback):
    ...
    def on_pre_evaluate_policy(self) -> None:
        ...
        # Detect a manipulated rigid object (e.g. the carried box) so the render
        # can show it alongside the robot. IsaacLab stores object state as
        # root_state_w = [pos(3), quat_wxyz(4), lin_vel(3), ang_vel(3)].
        self._object = None
        try:
            scene = getattr(sim, "scene", None)
            rigid_objects = getattr(scene, "rigid_objects", None) if scene is not None else None
            if rigid_objects:
                for oname, obj in rigid_objects.items():
                    if oname in ("usd_scene_objects",):
                        continue
                    self._object = obj
                    self._metadata["object_name"] = oname
                    break
        except Exception:
            self._object = None
        if self._object is not None:
            self._buffers["object_pos"] = []
            self._buffers["object_quat_wxyz"] = []
    ...
    def on_post_eval_env_step(self, actor_state: dict) -> dict:
        ...
        # Manipulated object (box) pose, if present.
        if getattr(self, "_object", None) is not None:
            try:
                obj_state = self._object.data.root_state_w[eid]
                self._buffers["object_pos"].append(_to_np(obj_state[:3]))
                self._buffers["object_quat_wxyz"].append(_to_np(obj_state[3:7]))
            except Exception:
                pass
