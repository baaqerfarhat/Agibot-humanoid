...
class JointPositionActionTerm(ActionTermBase):
    ...
    def __init__(self, cfg: ActionTermCfg, env: Any):
        ...
        self._configure_action_clip(env)
    ...
    def process_actions(self, actions: torch.Tensor) -> None:
        ...
        if self.env.robot_config.control.clip_actions:
            clip_limit = self.action_clip_limits
    ...
    def _configure_action_clip(self, env: Any) -> None:
        """Build the per-DOF action clip bound.

        Starts from the scalar ``action_clip_value`` and applies any per-joint
        overrides whose key is a substring of the DOF name, mirroring how the
        on-robot deployment selects the joints it bounds.
        """
        control_cfg = env.robot_config.control
        self.action_clip_limits = torch.full(
            (self._action_dim,), float(control_cfg.action_clip_value), device=env.device
        )

        overrides = getattr(control_cfg, "action_clip_value_by_joint", None)
        if not overrides:
            return

        matched: dict[str, list[str]] = {key: [] for key in overrides}
        for i, name in enumerate(env.dof_names):
            for key, value in overrides.items():
                if key in name:
                    self.action_clip_limits[i] = float(value)
                    matched[key].append(name)

        unmatched = [key for key, names in matched.items() if not names]
        if unmatched:
            raise ValueError(
                f"action_clip_value_by_joint keys matched no DOF: {unmatched}. "
                f"Available DOFs: {list(env.dof_names)}"
            )
    ...
    def _configure_action_scales(self, env: Any) -> None:
        ...
        overrides = getattr(control_cfg, "action_scale_overrides", None)
        if overrides:
            dof_names = env.robot_config.dof_names
            for key, value in overrides.items():
                for i, dof_name in enumerate(dof_names):
                    if key in dof_name:
                        self.action_scales[i] = value
