...
class AdaptiveTimestepsSampler:
    ...
    def sampling_probabilities(self) -> torch.Tensor:
        # Normalize failure counts into a distribution BEFORE mixing with the
        # uniform floor. The previous formulation added `ratio / num_bins` to raw
        # EMA failure *counts*, whose magnitude scales with num_envs (with 4096
        # envs the counts are ~100x the floor), so the uniform term was
        # negligible and one hard bin could absorb ~95% of all episode starts,
        # catastrophically starving the rest of the motion. As a true mixture,
        # `adaptive_uniform_ratio` now caps concentration regardless of scale.
        adaptive = self.bin_failed_count
        total = adaptive.sum()
        if total > 0:
            adaptive = adaptive / total
        else:
            adaptive = torch.full_like(adaptive, 1.0 / self.num_bins)
        sampling_probabilities = (
            1.0 - self.adaptive_uniform_ratio
        ) * adaptive + self.adaptive_uniform_ratio / float(self.num_bins)
    ...
    def setup(self) -> None:
        ...
        if self.motion_cfg.use_adaptive_timesteps_sampler:
            self.adaptive_timesteps_sampler = AdaptiveTimestepsSampler(
                ...
                self.device,
                int(1 / (self._env.dt)),
                adaptive_uniform_ratio=getattr(self.motion_cfg, "adaptive_uniform_ratio", 0.1),
            )
