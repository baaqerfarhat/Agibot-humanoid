...
class RobotControlConfig:
    ...
    # Per-joint action_scale overrides, matched by substring on the dof name.
    # Needed because action_scale = cfg * effort / kp gives a joint authority
    # proportional to its actuator, not to the range its task requires. On
    # ankle_roll that is 0.06 rad/unit, letting the policy command +35 deg on a
    # joint whose reference spans -5.2..+4.2 deg -- an offset the foot cannot
    # follow while in ground contact, so the PD pins at the 24 N-m limit for
    # 99% of the episode. Raising kp does NOT fix this: action_scale shrinks
    # proportionally, so the action-to-torque gain a*cfg*effort is unchanged.
