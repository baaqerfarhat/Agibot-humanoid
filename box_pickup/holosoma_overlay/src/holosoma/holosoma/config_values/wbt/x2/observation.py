"""Whole Body Tracking observation presets for the AgiBot X2 robot.

Structurally identical to the G1 preset: the object state is only exposed to the
critic (asymmetric actor-critic), so the deployed actor policy never depends on
privileged object measurements.
"""
...
x2_31dof_wbt_observation_w_object = ObservationManagerCfg(
    groups={
        "actor_obs": actor_obs_shared,
        "critic_obs": ObsGroupCfg(
            concatenate=True,
