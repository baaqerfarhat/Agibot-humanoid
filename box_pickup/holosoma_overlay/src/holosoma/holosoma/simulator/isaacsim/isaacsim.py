    ...
    def __init__(self, tyro_config: FullSimConfig, terrain_manager: TerrainManager, device: str):
        ...
        sim_config: SimulationCfg = SimulationCfg(
            ...
            physx=PhysxCfg(
                ...
                # GPU broadphase/contact buffer capacities. IsaacLab defaults are very
                # large (e.g. gpu_max_rigid_contact_count=2**23 -> ~640MB single buffer),
                # which OOMs on a shared/low-headroom GPU. These reduced values are
                # sufficient for the X2 box-carry task (1 articulation + 1 rigid object
                # per env) and can be raised via HOLOSOMA_PHYSX_SCALE if more envs/contacts
                # are needed.
                gpu_found_lost_pairs_capacity=int(os.environ.get("HOLOSOMA_GPU_FOUND_LOST_PAIRS", 2**18)),
                gpu_found_lost_aggregate_pairs_capacity=int(
                    os.environ.get("HOLOSOMA_GPU_FOUND_LOST_AGG_PAIRS", 2**20)
                ),
                gpu_total_aggregate_pairs_capacity=int(os.environ.get("HOLOSOMA_GPU_TOTAL_AGG_PAIRS", 2**18)),
                gpu_max_rigid_contact_count=int(os.environ.get("HOLOSOMA_GPU_MAX_RIGID_CONTACT", 2**20)),
                gpu_collision_stack_size=int(os.environ.get("HOLOSOMA_GPU_COLLISION_STACK", 2**26)),
                gpu_heap_capacity=int(os.environ.get("HOLOSOMA_GPU_HEAP", 2**24)),
                gpu_temp_buffer_capacity=int(os.environ.get("HOLOSOMA_GPU_TEMP_BUFFER", 2**22)),
            ),
