...
def load_motion_data(
    ...
    # Optimizer warm-up: the per-frame QP starts from the robot's default pose
    # and needs ~10-20 frames to converge onto the human, which otherwise bakes
    # a violent artificial transient into the start of the clip (root ang vel
    # up to ~4.6 rad/s at frame 0). Prepend repeats of the first frame so the
    # optimizer settles on the true (standing) start pose during frames that
    # are then discarded downstream (RETARGET_WARMUP_TRIM in the converter).
    warmup = int(os.environ.get("RETARGET_WARMUP_FRAMES", "20"))
    if warmup > 0:
        human_joints = np.concatenate([np.repeat(human_joints[:1], warmup, axis=0), human_joints], axis=0)
        object_poses = np.concatenate([np.repeat(object_poses[:1], warmup, axis=0), object_poses], axis=0)
        logger.debug("Prepended %d warm-up repeats of frame 0", warmup)
