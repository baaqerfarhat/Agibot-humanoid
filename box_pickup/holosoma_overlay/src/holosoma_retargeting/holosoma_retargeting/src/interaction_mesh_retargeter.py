...
class InteractionMeshRetargeter:
    ...
    def __init__(
        ...
        # Keep hinge/slide joints only. Filtering by name alone is WRONG when the
        # robot's freejoint is named (X2's is "floating_base_joint"): the freejoint
        # then stays in the list and shifts every joint's limits by one slot, so
        # each joint gets clamped to the PREVIOUS joint's range (e.g. shoulder
        # pitch pinned to waist roll's +/-0.488, left hip pitch pinned to the
        # freejoint's [0, 0]).
        actuated_joints = [
            (i, name)
