...
visits, with every suite individually significant either way; across 2,240 paired episodes in 110 stored cells
(faults, severities, profiles, controls and ablations, tallied by a script from the stored
outcomes) the correction fixes 625 and breaks 66. The same law repairs two further backbones from two other developers (OpenVLA-OFT,
...
\item \textbf{Evidence at scale, paired} (\S\ref{sec:exp}): four suites, three backbones, four
  ...
  healthy control on every cell, and 2,240 paired episodes in aggregate, tallied by script.
...
Cartesian increment plus gripper, executed by the benchmark's controller; for ALOHA it is
fourteen and for the GR1 humanoid twenty-nine absolute joint targets executed by position
servos. The measured state $y_t$ is the end-effector increment or the joint position.
...
(\S\ref{sec:aloha}), and the GR1 humanoid both ways on two tasks (\S\ref{sec:gr1}).
...
headline cells evaluate; the preregistered recalibration on disjoint states is the right-hand
column of Table~1 and Appendix~\ref{app:heldout}.
...
repaired to $40\%$ with no regression; milder friction is not a claim ($p=0.51$): friction
...
reaches a mean residual inside that margin and still fails, because its correction moves by
$0.43$\,cm within an episode, the whole margin. The same number, applied stationary,
repairs. \emph{Identify, then hold} (adapt during one sacrificial episode, hold thereafter)
...
the law scores $19/30$ vs $18/30$ and $16/30$ vs $15/30$ (7 fixed, 6 broken each, $p=1$);
...
restore; SimplerEnv's Google robot under base OpenVLA (planner-interpolated 3\,Hz deltas,
$R^2=0.57$, healthy $2/10$, a $+0.02$ offset at $20$--$60\%$ of its residual signature) fails
both conditions and no cell was run. \textbf{Harm on a healthy robot.} One OFT episode in
twenty was lost to a sustained phantom acted on; a dwell gate removes most spurious openings,
not that one. Three explanations we proposed were refuted by our own measurements and are kept
with what killed them (Appendix~\ref{app:refuted}).
...
sensor changes, not the law. Where the controller keeps the transient the deployable scheme is
identify-and-hold, a weaker claim than online adaptation. Policy stochasticity gives $\pm10$
points at $n=20$. We give propositions, not a recovery theorem. And every constant is a ratio
...
A frozen VLA can be repaired within one episode by six numbers at its action interface, given
a plant model and a sensitivity matrix identified once on healthy data. One calibration serves
four suites and three backbones, the two conditions that govern success are measurable in
advance, and the failures are named. The result still owed is the one on a real robot.
...
lower on $y$ and $z$, refuting the registered $15\%$ band on translation: the sensitivity
depends on the configuration the probe runs from. Condition number $3.1$ against $3.0$. The
...
vs the healthy $14/20$ on the same seeds $p=1.0$); held estimate $93$--$100\%$ of the fault.
