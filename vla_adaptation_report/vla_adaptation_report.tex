...
\textbf{across four LIBERO suites, $26\% \rightarrow 65\%$ pooled on paired
episodes, exact McNemar $p = 9.3\times 10^{-10}$, with zero regressions.}
...
\vspace{0.3em}
Across \textbf{450 paired episodes} --- four suites, four fault families, three
severities, four time profiles --- the correction fixes \textbf{158} episodes and
breaks \textbf{5} (1.1\,\%). A tuned integral baseline on the same fault reaches
...
\code{libero\_goal}    & 8/20 & 13/20 &  5 & 0 & 0.0625 \\
\code{libero\_object}  & 5/20 & 16/20 & 11 & 0 & $\mathbf{0.00098}$ \\
\code{libero\_10}      & 0/20 &  5/20 &  5 & 0 & 0.0625 \\
\midrule
\textbf{pooled} & \textbf{21/80 = 26\%} & \textbf{52/80 = 65\%} & \textbf{31} & \textbf{0}
  & $\mathbf{9.3\times 10^{-10}}$ \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{The result only paired data could show.}
\textbf{Frozen-only wins $= 0$}, in all four suites, across 80 paired episodes. Every
episode the frozen policy solved, the corrected policy also solved: the correction fixed
thirty-one episodes and broke none. Unpaired totals are equally consistent with a method
...
\paragraph{Two suites are power-limited, not weak.}
\code{goal} and \code{libero\_10} both land at exactly $p = 0.0625$. That is the
\emph{floor} of the exact test: with 5 discordant pairs all favouring the correction,
$2^{-5}\times 2 = 0.0625$ is the smallest attainable value, and a perfect result cannot do
better. Reaching $p < 0.05$ needs $\geq 6$ discordant pairs, i.e.\ $n \approx 30$--$40$ at
these effect sizes. That is a sample-size decision, not a finding.
...
\begin{enumerate}[leftmargin=1.6em,itemsep=3pt]
  ...
  \item \textbf{Four suites, paired}: $21/80 = 26\,\% \to 52/80 = 65\,\%$, exact
        McNemar $p = 9.3\times 10^{-10}$, with \textbf{zero regressions} --- the
        correction never broke an episode the frozen policy solved.
  \item \textbf{Every fault family tested is repaired}: constant additive
        ...
        loss). Across \textbf{450 paired episodes: 158 fixed, 5 broken}
        (1.1\,\% regression) --- and the three headline cells replicate on a
