...
headline cells evaluate; its preregistered recalibration on
disjoint states is Table~1's right-hand column.
...
Every suite is individually significant in both columns (held-out: $0.0039$, $0.0063$,
$0.039$, $0.016$; 37 fixed, 3 broken pooled). The single regression in the left column is one
...
state 45, which these cells evaluate. Re-identifying plant and $M$ at states from 25 upward
leaves three suites within one episode and costs the long-horizon suite half its repair,
$15/40\to7/40$ (preregistered; the drop refutes the registered bound of five,
Appendix~\ref{app:heldout}). A $20\%$ error in $M$'s translation block has two hundred steps to
accumulate on \code{libero\_10} and forty on the others: the cost of calibrating away from the
deployment state. \code{libero\_90}, on which the checkpoint was never fine-tuned,
scores $6/20$ unfaulted and is a competence floor, not a repair result.
...
remove without delaying genuine repair by one to two seconds (\S\ref{sec:fail}). The three
cells replicate on \code{libero\_object} with the same calibration ($0/20\to10/20$,
$0/20\to15/20$, $0/20\to19/20$).
...
returns $0/20$ at every severity, warm-started or cold. The reason is quantitative:
...
within an episode, but movement is not the cause: with the initial estimate matched across
arms, the innovation law updating throughout moves $0.30$\,cm and repairs $14/40$, the held
estimate $15/40$, the legacy law $0/40$. The legacy law's estimate is pulled $20\%$ below the
fault by step 20, a $0.4$\,cm residual during the grasp, and a correct estimate is not its
fixed point; the innovation law's is. The same number, applied stationary, repairs. \emph{Identify, then hold} (adapt during one sacrificial episode, hold thereafter)
