...
% ICLR-style geometry; swap for the official iclr2027_conference.sty when porting.
...
\providecommand{\citep}[1]{[\ref{#1}]}
\setlength{\parskip}{3pt}

\title{Frozen Yet Adaptive: Single-Episode Repair of Vision--Language--Action Policies from Proprioceptive Residuals}
\author{Anonymous authors\thanks{Draft generated from the project record on 2026-09-05. Numbers are verified against the stored per-episode outcomes; the record (\code{docs/ADAPTIVE\_CONTROL\_VLA.md}) keeps every refuted explanation in place.}}
\date{}
...
breaks 7. The same law and the \emph{same} calibration repair a second backbone
(OpenVLA-OFT) and, in joint space, a second manipulator (bimanual ALOHA). A tuned integral
...
\item \textbf{Evidence at scale, paired} (\S\ref{sec:exp}): four suites, two backbones, two
...
\subsection{A second backbone, unchanged calibration}
\label{sec:oft}

OpenVLA-OFT (7B, PyTorch, L1 action head) is served behind the same interface; every
script, the plant and $M$ are the $\pi_{0.5}$ ones.
...
\bibitem{jparc2026} M.~Jo, T.~Kwon, J.~Chun, Y.~Jeong, T.~Kim. Uncovering vulnerability of
vision-language-action models under joint-level physical faults. arXiv:2606.10501, 2026.
\bibitem{clae2026} S.~Das, D.~Chiu, S.~Hegde, G.~S.~Sukhatme. Steering multirobot behavior via
closed-loop affine activation editing. arXiv:2606.11489, 2026.
\end{thebibliography}

\appendix
\section{Reproducibility}
Every number above is recomputed from stored per-episode outcomes by \code{openpi/mcnemar.py}
with nothing installed; experiments need openpi at commit \code{15a9616}, the
\code{pi05\_libero} checkpoint, and one GPU. The two-virtualenv split, the OpenVLA-OFT and
ALOHA environments, and the three pins that were not obvious are in \code{SETUP.md}.
