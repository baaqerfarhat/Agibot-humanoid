...
against reference rollouts. Both need the fault to be represented in a dataset before it can
be corrected. Neither is available to a robot that degrades while running.
...
\textbf{Residual and offline calibration.} Learned residual policies and offline
calibration against reference rollouts correct systematic action error but require the
faulted behaviour in a dataset first. A closely related contemporary line (J-PARC, reported
to evaluate frozen $\pi_{0.5}$ and OpenVLA-OFT across LIBERO with learned residual
calibration) was cited to us in review; we have not verified its content and do not
characterise it further here. Closed-loop affine activation editing (CLAE) steers a policy
by editing internal activations against an observed objective; our correction lives
outside the network, at the action interface, and needs no access to activations.
...
actuator fault enters below it with a state-dependent map, and the hardware section is the
only place that fault class appears, without a repair result yet. The estimate resets every
