...
two other developers (OpenVLA-OFT, NVIDIA GR00T N1.7) and, in joint space, two further
manipulators: a bimanual ALOHA and a humanoid (Fourier GR1 under GR00T N1.5, $7\%\to70\%$,
its healthy rate). A tuned integral
...
\label{sec:gr1}
Fourier GR1 upper body (two 7-joint arms, two 6-joint dexterous hands, a 3-joint waist;
29 absolute joint targets at 20\,Hz) in NVIDIA's RoboCasa GR1 tabletop simulator, driven
zero-shot by GR00T N1.5, for which GR1 is a pretrained embodiment. Plant from six healthy
...
policy on the plate-to-plate task. Thirty paired episodes with a healthy arm in the same
process (scenes repeat per seed only within a process):

\begin{center}\small
\begin{tabular}{lrrrrrc}
\toprule
scheme & healthy & frozen & corrected & fixed & broken & McNemar \\
\midrule
continuous adaptation & 23/30 & 1/30 & 8/30 & 8 & 1 & $0.039$ \\
identify episode 0, then hold & 21/30 & 2/30 & \textbf{21/30} & 19 & 0 & $3.8\times10^{-6}$ \\
\bottomrule
\end{tabular}
\end{center}

Held, the estimate ($83$--$118\%$ of the fault per joint) takes the humanoid from $7\%$ to
its healthy rate ($p=1$ against healthy); updating, the same estimate reaches a third of
it and is significantly below healthy ($p=0.0015$). Proposition~\ref{prop:repair} decides
the scheme on the third manipulator as on the second.
