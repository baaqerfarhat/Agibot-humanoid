...
humanoid (Fourier GR1 under GR00T N1.5: on two tasks, $3\%\to63\%$ and $3\%\to43\%$, each its
healthy rate, zero regressions in sixty paired episodes). A tuned integral
...
\item \textbf{A single-episode repair loop for frozen VLAs} (\S\ref{sec:method}) needing
  ...
  episode and reconverges within a few tens of control steps; on the two joint-space
  manipulators it is identified in one to three episodes and held.
\item \textbf{Evidence at scale, paired} (\S\ref{sec:exp}): four suites, three backbones, three
  ...
  episode-for-episode against the frozen policy with exact McNemar tests, a healthy
  control on every cell, and over 1,300 paired episodes in aggregate across cells, controls
  and ablations.
...
see identical scenes. The estimate is read in the reach window (steps 50--200, before the
hand is on the plate) over three episodes and the median held: one episode's end reads a
...
reaches $8/30$ on plate-to-plate, a third of the ceiling: Proposition~\ref{prop:repair}
decides the scheme here as on ALOHA. Appendix~\ref{app:gr1map} maps three fault magnitudes.
...
showed. On hardware a stiffness loss is invisible in position and recoverable in effort, so the
