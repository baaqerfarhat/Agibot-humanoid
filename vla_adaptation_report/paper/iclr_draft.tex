...
class in the MuJoCo model of the LIBERO Panda, below the operational-space controller:
a constant joint torque (a payload or a gravity-compensation error), added joint friction,
and a joint lock. Magnitudes were measured first: at the elbow, 5\,N\,m leaks through the
controller as an 8--9\,cm drift over 40 steps whether or not the arm is commanded (an
additive fault from below), while a $\pm0.05$\,rad lock and $+2.0$ friction do nothing
while holding and remove 3--10\,cm of commanded motion (a loss of effectiveness from
below). An estimate-only probe identifies every one of them on the translation channels
and nowhere else, the lock at 15--23 healthy-phantom standard deviations. Paired cells,
$\pi_{0.5}$, \code{libero\_spatial}, translation corrected, healthy phantom subtracted:
...
and wrong for the one the arm moves into, and a tracking law follows that with a lag. The
lock is identified, corrected and not repaired: the estimate converges within 30 steps on
every episode and the correction changes nothing, because a lock removes a degree of
freedom and no additive command moves the arm along a direction it can no longer produce.
Identifiability is necessary for repair and not sufficient. J-PARC's offline residual
...
($18/20$ both arms, rotation phantom below $0.005$); the rotation $0.10$ fault takes it to
$0/20$ and the correction recovers $\mathbf{14/20}$ (14 fixed, 0 broken, $p=1.2\times10^{-4}$)
with no retuning, the estimate settling at $81\%$, $42\%$ and $82\%$ of the true fault. The under-identified $r_y$ is the same channel that lags on the other two
...
Faults below the controller are repaired only when they act as inputs: a joint torque
bias is (with a regression rate the Cartesian cells never showed), a joint lock is not
(\S\ref{sec:joint}), and friction at the levels that identify does not damage this policy.
A joint-space law would see the torque bias as the constant it is; the Cartesian residual
sees it through the Jacobian. The X2 logs say a stiffness loss is invisible in position and
recoverable in effort, which suggests the sensor, not the law, is what changes on hardware. The estimate resets every
...
is paired, controlled and broad; the calibration transfers across suites and three
backbones; the two conditions that govern success are measurable in advance; the failures
