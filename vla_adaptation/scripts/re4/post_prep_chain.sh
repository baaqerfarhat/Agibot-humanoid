...
echo "#### E1 pass1 fit+qual (init39, episodes 0-9)  $(date +%T)"; $PY $REPO/openpi/re4_theory/e1_continuations.py --log $SRC/e1_fitqual_spatial_init39.json --episodes all --checkpoints 20,40 --horizon 50 --split-label fit_qualification --out $R/E1/pass1_fitqual.json > $R/E1/pass1_fitqual.log 2>&1; echo "exit $?"; echo "RUN DONE E1 pass1"
HOLD=$(python3 - <<'PY'
...
echo "#### E1 pass2 locked test (init34)  $(date +%T)"; $PY $REPO/openpi/re4_theory/e1_continuations.py --log $SRC/e1_test_spatial_init34.json --episodes all --checkpoints 20,40 --horizon 50 --hold-estimate $HOLD --split-label locked_test --out $R/E1/pass2_test.json > $R/E1/pass2_test.log 2>&1; echo "exit $?"; echo "RUN DONE E1 pass2"
