...
cd $OPENPI && CUDA_VISIBLE_DEVICES=0 XLA_PYTHON_CLIENT_MEM_FRACTION=0.7 nohup $OPENPI/.venv/bin/python /home/mtaheri/ws_AgibotX2/Agibot-humanoid/vla_adaptation/openpi/ace_server.py --port 8001 --control $SP/ctl0.json --ack $SP/ack0.json > $SP/pi05_server_six0.log 2>&1 &
SERVER_PID=$!; t=0; until grep -q "listening" $SP/pi05_server_six0.log 2>/dev/null; do sleep 10; t=$((t+10)); if [ $t -gt 1500 ]; then echo "SERVER NOT READY"; exit 1; fi; done
...
BASE="--port 8001 --control $SP/ctl0.json --ack $SP/ack0.json $LAW $FIT --suite libero_10 --manifest $MAN/libero_10_E2_core.json"
...
go libero_10_healthy_U --sev 0.0 --arms adaptive
go libero_10_healthy_C --sev 0.0 --arms adaptive --dc-gain 4=0.254
kill $SERVER_PID; echo "SIX GPU0 DONE $(date +%T)"
