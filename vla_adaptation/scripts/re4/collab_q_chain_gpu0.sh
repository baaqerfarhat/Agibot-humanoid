...
mkdir -p $R
cd $OPENPI && CUDA_VISIBLE_DEVICES=0 XLA_PYTHON_CLIENT_MEM_FRACTION=0.7 nohup $OPENPI/.venv/bin/python /home/mtaheri/ws_AgibotX2/Agibot-humanoid/vla_adaptation/openpi/ace_server.py --port 8001 --control $SP/ctl0.json --ack $SP/ack0.json > $SP/pi05_server_q0.log 2>&1 &
SERVER_PID=$!
t=0; until grep -q "listening" $SP/pi05_server_q0.log 2>/dev/null; do sleep 10; t=$((t+10)); if [ $t -gt 1500 ]; then echo "SERVER NOT READY"; exit 1; fi; done
...
go q2_C_libero_10  libero_10 --manifest $MAN/libero_10_E2_core.json --sev 0.05 --arms adaptive --law innov --dc-constrain corrected
