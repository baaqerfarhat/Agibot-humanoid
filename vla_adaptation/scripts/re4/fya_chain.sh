#!/bin/bash
# FrozenYet Adaptive recovery campaign (prereg_records/PREREG_FYA_RECOVERY_DEADLINE_V1.md), Stages 1 and 2.
...
# ---------------- A. fresh healthy sources (policy inference) ----------------
stamp "A sources: server up"; server_up sources || exit 1
stamp "A qualification sources states 9,10 (seeds 81000+2t+r)"
timeout 7200 $PY -u $REPO/openpi/error_signal.py --port $PORT --control $CTL --ack $ACK --suite libero_spatial --healthy-only --init-base 9 --episodes 20 \
  --sampler-seeds "$QSEEDS" --out $R/sources/qualification_spatial_init9_10.json > $R/sources/qualification_spatial_init9_10.log 2>&1; echo "exit $?"
