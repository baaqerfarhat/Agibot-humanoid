...
Jall = view.get_jacobians()[0].detach().cpu().numpy().astype(float)
...
# IsaacLab's jacobian body index EXCLUDES the fixed root when the base is floating:
# body i of body_names may map to row i-1. Test both, x both row conventions.
def score(G, lab):
    cs = [float(f@(G@d)/(np.linalg.norm(f)*np.linalg.norm(G@d)+1e-300)) for f,d in zip(FD,dirs)]
    ln = [float(f[:3]@(G@d)[:3]/(np.linalg.norm(f[:3])*np.linalg.norm((G@d)[:3])+1e-300)) for f,d in zip(FD,dirs)]
    rt = [float(np.linalg.norm(f)/(np.linalg.norm(G@d)+1e-300)) for f,d in zip(FD,dirs)]
    print(f"  {lab:<44} cos {np.mean(cs):+.4f}  lin {np.mean(ln):+.4f}  ratio {np.median(rt):.4f}")
    return np.mean(cs)
print(f"\njacobian tensor bodies = {Jall.shape[0]}, body_names = {len(bodies)}")
best = (-9, None)
for boff, blab in ((0, "row=idx"), (-1, "row=idx-1")):
    bi = HAND + boff
    if not (0 <= bi < Jall.shape[0]): continue
    Jh = Jall[bi]
    for swap, slab in ((False, "[lin;ang]"), (True, "[ang;lin]")):
        Jh2 = np.vstack([Jh[3:], Jh[:3]]) if swap else Jh
        G = Jh2 @ Minv @ St
        c = score(G, f"{blab}, {slab}")
        if c > best[0]: best = (c, f"{blab}, {slab}")
print(f"\nbest: {best[1]}  cos {best[0]:+.4f}")
