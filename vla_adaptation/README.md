...
| **descriptor-form controller with ARX state identification** (`learned_adaptation/`, an attributed experimental module by the audit's author) | seven Spatial joint-torque cells 112/140 → 135/140 and Object joint 3 7/20 → 18/20 at a correction cap of 0.25 chosen after Object development outcomes (12/20 at the registered cap 0.15, p = 0.23) against their own frozen controls, constant basis Φ = E, no trained features; its composite tracking term was numerically inert, so it is a prediction-only law (author's correction, 2026-09-15); not a matched comparison with the estimators above (`SWEEP_RESULT.md`) |
...
| **constants by the paper's rule** | innovation law with the normaliser over the corrected channels, four suites: pooled 29→79/120 (51 fixed, 1 broken) against the shipped 28→78; estimate at the truth on r_z, 85–93 % on r_x, r_y 42–60 % under every law; healthy 118→115 against 116→117. Not adopted by the registered rule (libero_10 13/40 vs ≥ 15); reported as an ablation row (§53) |
| **the r_y lever** | plant refitted with its rotation tap sums pinned to the probed M (`--dc-constrain corrected`): r_y estimate 41 → 82 % (spatial), 45 → 70.5 % (libero_10); libero_10 0 → 18/40 (shipped 15, oracle 22; paired 10 won / 7 lost); spatial 17/20; healthy 20 → 19. Registered predictions hold; the DC-gain diagnosis confirmed by intervention (§54) |
| **matched baselines under one protocol** | static observer (K = 0) equal on spatial, six of forty worse on libero_10 (p = 0.07) but indistinguishable at n = 80 with the sampler pinned (9/80 vs 10/80, p = 1; §51); innovation law within three of the legacy law; the known-fault oracle is the ceiling, 19/20 and 22/40, and the method reaches 109 % and 71 % of it; exact −f on all six channels 20/20 (§44) |
